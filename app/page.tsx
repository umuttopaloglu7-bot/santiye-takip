...
// Build sırasında statik dosya oluşturmayı ve hata vermeyi kesin olarak kapatır
...
export default function PuantajYonetim() {
  ...
  const verileriGetir = async () => {
    // Eğer bağlantı bilgileri yoksa Supabase'e gitme (Build hatasını önler)
  ...
  async function puantajKaydet(tip: string, deger?: number) {
    ...
    if (tip === 'sil') {
      ...
      // Upsert işlemi: Hem saat hem mesai sütunlarını doldurarak kesin işaretleme sağlar
    ...
    return (
      <main className="min-h-screen bg-[#02040a] flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-[#0b101d] p-12 rounded-[3rem] border border-slate-800 shadow-2xl text-center">
          <ShieldCheck size={40} className="mx-auto mb-6 text-blue-500"/><h1 className="text-3xl font-black text-white italic uppercase">ŞANTİYE TAKİP</h1>
        ...
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-md">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] w-full max-w-md border border-slate-700 shadow-2xl text-center">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <button onClick={() => puantajKaydet('tam')} className="bg-green-600 p-6 rounded-[1.5rem] font-black text-white hover:bg-green-500 flex flex-col items-center gap-2"><Check size={24}/><span>TAM GÜN</span></button>
      ...
      {showUstaModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[110]">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] border border-slate-700 shadow-2xl w-full max-w-sm text-center">
            ...
            <button onClick={ustaEkle} className="w-full bg-blue-600 p-5 rounded-2xl font-black text-white hover:bg-blue-500 uppercase">KAYDET</button>
