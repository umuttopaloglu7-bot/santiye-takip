...
import { Plus, Check, Construction, Calculator, FileSpreadsheet, LayoutDashboard, Users, Lock, LogOut, ShieldCheck, Trash2, ChevronLeft, ChevronRight, Download, Clock, Plane } from 'lucide-react';
...
export default function PuantajYonetim() {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) {
      verileriGetir();
      const kanal = supabase.channel('pano_takip').on('postgres_changes', { event: '*', schema: 'public', table: 'puantaj' }, () => syncVeri()).subscribe();
  ...
  async function puantajKaydet(tip: string, deger?: number) {
    ...
    if (tip === 'sil') {
      ...
      // Hem 'saat' hem de 'mesai' sütununu doldurarak hata riskini sıfıra indiriyoruz
  ...
  if (!isLoggedIn) {
    return (
      <main className="min-h-screen bg-[#02040a] flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-[#0b101d] p-12 rounded-[3rem] border border-slate-800 shadow-2xl text-center">
          <ShieldCheck size={40} className="mx-auto mb-6 text-blue-500"/><h1 className="text-3xl font-black text-white italic">ŞANTİYE TAKİP</h1>
          <input type="password" autoFocus placeholder="ŞİFRE" className="w-full bg-[#161b2c] border border-slate-700 p-6 rounded-2xl text-white text-center font-black my-6" value={loginSifre} onChange={(e) => setLoginSifre(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (loginSifre === ADMIN_SIFRE ? setIsLoggedIn(true) : alert("Hatalı"))}/>
          <button onClick={() => loginSifre === ADMIN_SIFRE ? setIsLoggedIn(true) : alert("Hatalı")} className="w-full bg-blue-600 p-6 rounded-2xl text-white font-black">GİRİŞ YAP</button>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#02040a] text-slate-300 p-6 font-sans uppercase text-[11px]">
      <div className="max-w-[1800px] mx-auto space-y-8">
        <div className="flex justify-between items-center bg-[#0b101d] p-4 rounded-[1.5rem] border border-slate-800/50">
           <div className="flex items-center gap-6">
              ...
              <div className="flex items-center bg-[#161b2c] rounded-xl border border-slate-700 overflow-hidden">
                <button onClick={() => ayDegistir('geri')} className="p-3 hover:bg-blue-600/20 text-blue-500"><ChevronLeft size={18}/></button>
                <span className="px-6 font-black text-white text-[12px] min-w-[150px] text-center">{ayAdi} {yil}</span>
                <button onClick={() => ayDegistir('ileri')} className="p-3 hover:bg-blue-600/20 text-blue-500"><ChevronRight size={18}/></button>
              </div>
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <button onClick={() => alert("Rapor şifresi: 1954")} className="bg-purple-600 p-8 rounded-[2rem] flex items-center justify-center gap-4 text-white font-black hover:bg-purple-500 shadow-xl shadow-purple-900/20"><Lock size={24}/> GENEL RAPOR</button>
        </div>
        ...
        <div className="flex items-center gap-4 bg-[#0b101d] p-4 rounded-[1.5rem] border border-slate-800/50 overflow-x-auto no-scrollbar">
          ...
          <div className="flex gap-2">
            {alanlar.map(alan => (
              <button key={alan.id} onClick={() => setAktifAlan(alan.ad)} className={`px-8 py-3 rounded-2xl font-black whitespace-nowrap transition-all ${aktifAlan === alan.ad ? "bg-blue-600 text-white shadow-lg" : "bg-[#161b2c] text-slate-500"}`}>{alan.ad}</button>
            ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-md">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] w-full max-w-md border border-slate-700 shadow-2xl text-center">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <button onClick={() => puantajKaydet('tam')} className="bg-green-600 p-6 rounded-[1.5rem] font-black text-white hover:bg-green-500 shadow-lg flex flex-col items-center gap-2"><Check size={24}/><span>TAM GÜN (8 Saat)</span></button>
              <button onClick={() => puantajKaydet('izin')} className="bg-slate-700 p-6 rounded-[1.5rem] font-black text-white hover:bg-slate-600 shadow-lg flex flex-col items-center gap-2"><Plane size={24}/><span>İZİNLİ/RAPORLU</span></button>
            </div>
            <div className="bg-[#161b2c] p-6 rounded-[2rem] border border-slate-700 mb-6 text-center">
               <p className="text-blue-500 font-black text-[10px] mb-4 uppercase text-center">SAAT VEYA MESAİ GİRİN</p>
               <div className="flex gap-4">
      ...
      {showUstaModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[110]">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] border border-slate-700 shadow-2xl w-full max-w-sm text-center">
            ...
            <button onClick={ustaEkle} className="w-full bg-blue-600 p-5 rounded-2xl font-black text-white hover:bg-blue-500 transition-all text-center">KAYDET</button>
