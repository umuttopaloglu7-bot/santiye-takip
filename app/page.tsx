...
// Build hatasını önleyen güvenli bağlantı
...
export default function PuantajYonetim() {
  ...
  async function puantajKaydet(tip: string, deger?: number) {
    ...
    setSeciliDetay(null);
    setSaatInput('');
    await verileriGetir(); // Veriyi anında tazele
  }
      ...
      <div className="max-w-[1800px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-[#0b101d] p-8 rounded-[2rem] border border-slate-800/50 flex items-center gap-6">
            ...
            <p className="text-3xl font-black text-white">{(puantajlar.reduce((acc, curr) => acc + (curr.saat > 0 ? curr.saat : (curr.mesai === 'tam' ? 8 : 0)), 0) / 8).toFixed(2)}</p></div>
          </div>
        ...
        <div className="flex items-center gap-4 bg-[#0b101d] p-4 rounded-[1.5rem] border border-slate-800/50 overflow-x-auto no-scrollbar">
          ...
          <div className="ml-auto flex gap-2 mr-4">
            {aktifAlan && <button onClick={alanSil} className="p-3 bg-red-600/10 text-red-500 rounded-2xl"><Trash2 size={24}/></button>}
            <button onClick={() => setShowAlanModal(true)} className="p-3 bg-blue-600/10 text-blue-500 rounded-2xl"><Plus size={24}/></button>
          </div>
        </div>
                ...
                {ustalar.filter(u => u.alan === aktifAlan).map(usta => (
                  <tr key={usta.id} className="border-t border-slate-800/50 group">
                    <td className="p-6 font-black sticky left-0 bg-[#0b101d] text-slate-200 z-10 flex items-center justify-between border-r border-slate-800">
                        <span>{usta.ad}</span>
                        <button onClick={() => ustaSil(usta.ad)} className="opacity-0 group-hover:opacity-100 text-red-500"><Trash2 size={16}/></button>
                    </td>
                    {gunler.map(g => {
        ...
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-md">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] w-full max-w-md border border-slate-700 shadow-2xl text-center">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <button onClick={() => puantajKaydet('tam')} className="bg-green-600 p-6 rounded-[1.5rem] font-black text-white flex flex-col items-center gap-2"><Check size={24}/><span>TAM GÜN (8 Saat)</span></button>
            ...
            <div className="bg-[#161b2c] p-6 rounded-[2rem] border border-slate-700 mb-6">
               ...
               <div className="flex gap-4">
                  <input type="number" placeholder="SAAT..." className="flex-1 bg-[#0b101d] border border-slate-700 p-4 rounded-xl text-white font-black text-center" value={saatInput} onChange={(e) => setSaatInput(e.target.value)} />
