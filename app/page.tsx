...
const STANDART_CALISMA_SAATI = 8; // 1 yevmiye kaç saat?
...
export default function PuantajYonetim() {
  ...
  async function puantajKaydet(tip: string, deger?: number) {
    ...
    if (tip === 'sil') {
      ...
      await supabase.from('puantaj').upsert({ 
        ...
        saat: kayitDegeri // 'mesai' sütunu yerine artık 'saat' üzerinden gidiyoruz
      });
    }
    setSeciliDetay(null); setSaatInput('');
  }
  ...
  const excelIndir = () => {
    ...
    const excelVerisi = aktifUstaListesi.map(usta => {
      ...
      return { 
        "ŞANTİYE": aktifAlan, "USTA ADI": usta.ad, 
        "TOPLAM ÇALIŞMA (SAAT)": toplamSaat,
        "TOPLAM YEVMİYE (8 SAAT BAZLI)": toplamSaat / STANDART_CALISMA_SAATI,
        "İZİNLİ/RAPORLU GÜN": izinliGun,
        "BİRİM YEVMİYE": 0, "TOPLAM HAKEDİŞ": 0 
      };
    });
    const ws = XLSX.utils.json_to_sheet(excelVerisi);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Aylık Rapor");
    XLSX.writeFile(wb, `${aktifAlan}_${ayAdi}_Raporu.xlsx`);
  };
  ...
  const santiyeTumZamanlarIndir = async () => {
    ...
    const excelVerisi = aktifUstaListesi.map(usta => {
      ...
      return { 
        "ŞANTİYE": aktifAlan, "USTA ADI": usta.ad, 
        "GENEL TOPLAM SAAT": toplamSaat,
        "GENEL TOPLAM YEVMİYE": toplamSaat / STANDART_CALISMA_SAATI,
        "BİRİM YEVMİYE": 0, "GENEL HAKEDİŞ": 0
      };
    });
    const ws = XLSX.utils.json_to_sheet(excelVerisi);
    const wb = XLSX.utils.book_new();
  ...
  const genelRaporIndir = () => {
    const genelVeri = alanlar.map(alan => {
      ...
      return { "ŞANTİYE": alan.ad, "USTA SAYISI": ustalar.filter(u => u.alan === alan.ad).length, "TOPLAM YEVMİYE": tSaat / STANDART_CALISMA_SAATI };
    });
      ...
      <div className="max-w-[1800px] mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center bg-[#0b101d] p-4 rounded-[1.5rem] border border-slate-800/50">
           <div className="flex items-center gap-6">
              <span className="ml-4 text-[9px] font-black text-slate-500 italic tracking-widest">SİSTEM ÇEVRİMİÇİ</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <button onClick={() => setShowSifreModal({tip: 'genel_ozet'})} className="bg-purple-600 p-8 rounded-[2rem] flex items-center justify-center gap-4 text-white font-black hover:bg-purple-500 shadow-xl shadow-purple-900/20">
        ...
        <div className="bg-[#0b101d] rounded-[2.5rem] border border-slate-800/50 overflow-hidden shadow-2xl">
          <div className="p-8 border-b border-slate-800/50 flex flex-wrap gap-4 justify-between items-center">
            ...
            <div className="flex gap-4 relative">
              <button onClick={() => setShowDownloadMenu(!showDownloadMenu)} className="bg-green-600 text-white px-8 py-4 rounded-2xl font-black flex items-center gap-3 hover:bg-green-500 shadow-lg"><Download size={20}/> İNDİR</button>
              {showDownloadMenu && (
                <div className="absolute top-full mt-2 right-0 w-64 bg-[#161b2c] border border-slate-700 rounded-2xl shadow-2xl z-50 overflow-hidden">
                  <button onClick={() => { setShowSifreModal({tip: 'aylik'}); setShowDownloadMenu(false); }} className="w-full text-left px-6 py-4 hover:bg-blue-600 text-white font-bold border-b border-slate-800/50">📅 {ayAdi} Raporu</button>
                    ...
                    {gunler.map(g => {
                      ...
                      return (
                        <td key={g} className={`p-2 border-r border-slate-800/20 ${['Cmt', 'Paz'].includes(gunAdi) ? 'bg-white/[0.01]' : ''}`}>
                          <button onClick={() => setSeciliDetay({ usta: usta.ad, gun: g })} className={`w-12 h-12 mx-auto rounded-xl border-2 flex items-center justify-center transition-all ${!p ? "border-slate-800/50 hover:border-slate-500" : p.saat === STANDART_CALISMA_SAATI ? "bg-green-600 border-green-400 text-white" : p.saat === -1 ? "bg-slate-700 border-slate-500 text-white" : "bg-orange-500 border-orange-300 text-white font-black text-[14px]"}`}>
      ...
      {showSifreModal && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-xl">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] border border-slate-700 text-center shadow-2xl w-full max-w-sm">
            <Lock className="mx-auto mb-6 text-blue-500" size={40}/><h2 className="text-2xl font-black text-white mb-6 uppercase">YETKİ GEREKLİ</h2>
            ...
            <button onClick={() => setShowSifreModal(null)} className="mt-4 text-slate-600 text-[10px] font-black uppercase">İPTAL</button>
          </div>
        </div>
      )}

      {/* SAAT BAZLI PUANTAJ GİRİŞ MODALI (GÜNCELLENDİ) */}
      {seciliDetay && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-md">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] w-full max-w-md border border-slate-700 shadow-2xl text-center">
            <h3 className="text-3xl font-black text-white mb-2 italic">{seciliDetay.usta}</h3>
            <p className="text-slate-500 mb-8 font-bold text-[10px]">{seciliDetay.gun} {ayAdi.toUpperCase()} {yil}</p>
            ...
            <button onClick={() => {setSeciliDetay(null); setSaatInput('');}} className="w-full text-slate-500 font-black hover:text-white transition-all text-[10px]">KAPAT</button>
          </div>
        </div>
      )}

      {/* Şantiye ve Usta Ekleme Modalları (Aynı kaldı) */}
      {showAlanModal && (
