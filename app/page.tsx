...
// Vercel üzerinden gelen gerçek URL ve KEY kullanılacak
...
export default function PuantajYonetim() {
  ...
  async function puantajKaydet(tip: string, deger?: number) {
    ...
    // Veritabanına gidecek saat değerini netleştiriyoruz
    ...
    try {
      ...
      setSeciliDetay(null);
      setSaatInput('');
      await verileriGetir(); // Tabloyu anında güncelle
    } catch (err) {
      alert("Kayıt yapılamadı! Supabase tablo ayarlarınızı (RLS) kontrol edin.");
    }
  }

  // Şantiye ve Usta ekleme/silme fonksiyonları
    ...
    return (
      <main className="min-h-screen bg-[#02040a] flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-[#0b101d] p-12 rounded-[3rem] border border-slate-800 shadow-2xl text-center">
          ...
          <input type="password" autoFocus className="w-full bg-[#161b2c] border border-slate-700 p-6 rounded-2xl text-white text-center font-black my-6 outline-none focus:border-blue-500" value={loginSifre} onChange={(e) => setLoginSifre(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (loginSifre === ADMIN_SIFRE ? setIsLoggedIn(true) : alert("Hatalı"))}/>
          <button onClick={() => loginSifre === ADMIN_SIFRE ? setIsLoggedIn(true) : alert("Hatalı")} className="w-full bg-blue-600 p-6 rounded-2xl text-white font-black hover:bg-blue-500 transition-all">GİRİŞ YAP</button>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#02040a] text-slate-300 p-6 font-sans uppercase text-[11px]">
      <div className="max-w-[1800px] mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center bg-[#0b101d] p-4 rounded-[1.5rem] border border-slate-800/50">
           ...
           <button onClick={() => setIsLoggedIn(false)} className="bg-red-600/10 text-red-500 px-6 py-2 rounded-xl font-black flex items-center gap-2">ÇIKIŞ <LogOut size={16}/></button>
        </div>
                ...
                {ustalar.filter(u => u.alan === aktifAlan).map(usta => (
                  <tr key={usta.id} className="border-t border-slate-800/50 group hover:bg-white/[0.02]">
                    ...
                    {gunler.map(g => {
                      ...
                      // RENK VE TİK AYARLARI BURADA YAPILIYOR
                      const isTamGun = p?.saat === 8 || p?.mesai === 'tam';
                      const isIzinli = p?.saat === -1 || p?.mesai === 'izin';
                      const isOzelSaat = p && !isTamGun && !isIzinli;
                      ...
                      return (
                        <td key={g} className="p-2 border-r border-slate-800/20 text-center">
                          <button onClick={() => setSeciliDetay({ usta: usta.ad, gun: g })} 
                            ...
                            {isTamGun ? <Check size={20}/> : isIzinli ? "İ" : p?.saat || ""}
                          </button>
                        </td>
                      );
                    })}
        ...
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-md">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] w-full max-w-md border border-slate-700 shadow-2xl text-center">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <button onClick={() => puantajKaydet('tam')} className="bg-green-600 p-6 rounded-[1.5rem] font-black text-white hover:bg-green-500 flex flex-col items-center gap-2"><Check size={24}/><span>TAM GÜN (8 Saat)</span></button>
              <button onClick={() => puantajKaydet('izin')} className="bg-slate-700 p-6 rounded-[1.5rem] font-black text-white hover:bg-slate-600 flex flex-col items-center gap-2"><Plane size={24}/><span>İZİNLİ/RAPORLU</span></button>
            </div>
            ...
            <button onClick={() => puantajKaydet('sil')} className="w-full bg-red-600/10 text-red-500 p-4 rounded-2xl font-black mb-4 hover:bg-red-600 hover:text-white uppercase italic">KAYDI SİL</button>
            <button onClick={() => {setSeciliDetay(null); setSaatInput('');}} className="w-full text-slate-500 font-black hover:text-white text-[10px] uppercase">KAPAT</button>
          </div>
        </div>
      )}

      {/* Yeni Şantiye/Usta Modalları */}
      {showAlanModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[110]">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] border border-slate-700 shadow-2xl w-full max-w-sm text-center">
            <h2 className="text-xl font-black text-white mb-6 uppercase italic">YENİ ŞANTİYE</h2>
      ...
      {showUstaModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[110]">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] border border-slate-700 shadow-2xl w-full max-w-sm text-center">
            <h2 className="text-xl font-black text-white mb-6 uppercase italic">YENİ USTA</h2>
