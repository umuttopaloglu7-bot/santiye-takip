...
export default function PuantajYonetim() {
  ...
  async function puantajKaydet(tip: string, deger?: number) {
    ...
    // İşaretleme Yapamama Sorununu Çözen Kritik Bölüm
    ...
    if (tip === 'sil') {
      ...
      // Upsert işlemi: Eğer usta, alan ve tarih eşleşiyorsa güncelle, yoksa ekle
      await supabase.from('puantaj').upsert({ 
        ...
        gun: seciliDetay.gun, 
        saat: kayitDegeri,
        mesai: tip === 'tam' ? 'tam' : 'ozel' // Eski yapıyla uyum için
      });
    }
    
    setSeciliDetay(null);
    setSaatInput('');
    verileriGetir(); // Veritabanından güncel halini anında çek
  }
    ...
    return (
      <main className="min-h-screen bg-[#02040a] flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-[#0b101d] p-12 rounded-[3rem] border border-slate-800 shadow-2xl text-center">
          ...
          <input type="password" autoFocus className="w-full bg-[#161b2c] border border-slate-700 p-6 rounded-2xl text-white text-center font-black my-6" value={loginSifre} onChange={(e) => setLoginSifre(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (loginSifre === ADMIN_SIFRE ? setIsLoggedIn(true) : alert("Hatalı"))}/>
          <button onClick={() => loginSifre === ADMIN_SIFRE ? setIsLoggedIn(true) : alert("Hatalı")} className="w-full bg-blue-600 p-6 rounded-2xl text-white font-black hover:bg-blue-500">GİRİŞ YAP</button>
        </div>
      </main>
    );
  }
