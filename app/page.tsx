...
// Supabase istemcisini sadece tarayıcıda (runtime) başlatan güvenli yapı
const getSupabase = () => {
...
export default function PuantajYonetim() {
  ...
  async function puantajKaydet(tip: string, deger?: number) {
    const supabase = getSupabase();
    if (!supabase || !seciliDetay || !aktifAlan) return;
  ...
  const genelRaporHazirla = async () => {
    ...
    excelIndir(rapor, "TUM_SANTIYELER_GENEL_DOKUM");
  };
  ...
  const santiyeRaporu = async (tip: 'aylik' | 'genel') => {
    ...
    if (!data) return;
    const rapor = data.map(p => ({ "Usta": p.usta, "Tarih": `${p.gun}/${p.ay}/${p.yil}`, "Çalışma": p.mesai === -1 ? "İzin" : p.mesai }));
    excelIndir(rapor, `${aktifAlan}_${tip === 'aylik' ? ayAdi : 'GENEL'}_PUANTAJ`);
    ...
    <main className="min-h-screen bg-[#02040a] text-slate-300 p-6 font-sans uppercase text-[11px]">
      <div className="max-w-[1800px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <button onClick={genelRaporHazirla} className="bg-purple-600 p-8 rounded-[2rem] flex items-center justify-center gap-4 text-white font-black hover:bg-purple-500 shadow-xl shadow-purple-900/20">
        ...
        {/* Ana Çizelge (1. Fotoğraftaki gibi yeşil tikli) */}
        <div className="bg-[#0b101d] rounded-[2.5rem] border border-slate-800/50 overflow-hidden shadow-2xl">
          <div className="p-8 border-b border-slate-800/50 flex justify-between items-center bg-white/[0.01]">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => setShowUstaModal(true)} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black flex items-center gap-3">
                ...
                {ustalar.filter(u => u.alan === aktifAlan).map(usta => (
                  <tr key={usta.id} className="border-t border-slate-800/50 group">
                    <td className="p-6 font-black sticky left-0 bg-[#0b101d] text-slate-200 z-10 flex items-center justify-between border-r border-slate-800">
                        <span>{usta.ad}</span>
                        <button onClick={async () => { if(confirm("Silinsin mi?")) { await supabase?.from('ustalar').delete().eq('id', usta.id); verileriGetir(); } }} className="opacity-0 group-hover:opacity-100 text-red-500"><Trash2 size={16}/></button>
                    </td>
                    {gunler.map(g => {
                      const p = puantajlar.find(px => px.usta === usta.ad && px.gun === g && px.alan === aktifAlan);
                      const isTam = p?.mesai === 8;
                      const isYarim = p?.mesai === 4;
                      const isIzin = p?.mesai === -1;
                      return (
                        <td key={g} className="p-2 border-r border-slate-800/20 text-center">
                          <button onClick={() => setSeciliDetay({ usta: usta.ad, gun: g })} className={`w-12 h-12 mx-auto rounded-xl border-2 flex items-center justify-center transition-all ${!p ? "border-slate-800/50" : isTam ? "bg-green-600 border-green-400 text-white" : isYarim ? "bg-orange-500 border-orange-300 text-white" : isIzin ? "bg-slate-700 border-slate-500 text-white" : "bg-orange-500 border-orange-300 text-white"}`}>
                            {isTam ? <Check size={20}/> : isYarim ? "1/2" : isIzin ? "İ" : p?.mesai || ""}
                          </button>
                        </td>
                      );
                    })}
        ...
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-md p-6">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] w-full max-w-md border border-slate-700 shadow-2xl text-center">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <button onClick={() => puantajKaydet('tam')} className="bg-green-600 p-6 rounded-[1.5rem] font-black text-white flex flex-col items-center gap-2"><Check size={24}/><span>TAM GÜN (8)</span></button>
      ...
      {showAlanModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[110]">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] border border-slate-700 w-full max-w-sm text-center">
            ...
            <button onClick={async () => { const s = getSupabase(); if(s) { await s.from('alanlar').insert([{ ad: yeniAlanAd }]); setYeniAlanAd(''); setShowAlanModal(false); verileriGetir(); } }} className="w-full bg-blue-600 p-5 rounded-2xl font-black text-white">OLUŞTUR</button>
          </div>
        </div>
      )}
      {showUstaModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[110]">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] border border-slate-700 w-full max-w-sm text-center">
            ...
            <button onClick={async () => { const s = getSupabase(); if(s) { await s.from('ustalar').insert([{ ad: yeniUstaAd, alan: aktifAlan }]); setYeniUstaAd(''); setShowUstaModal(false); verileriGetir(); } }} className="w-full bg-blue-600 p-5 rounded-2xl font-black text-white">KAYDET</button>
          </div>
        </div>
      )}
