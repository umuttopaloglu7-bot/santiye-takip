...
// Build sırasında statik oluşturmayı kesin olarak kapatıyoruz
...
// Supabase istemcisini sadece tarayıcıda başlatan güvenli fonksiyon
const getSupabase = () => {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
  const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "";
  // Eğer bilgiler yoksa bile hata fırlatma, null dön (build'i kurtarır)
...
export default function PuantajYonetim() {
  ...
  async function alanEkle() {
    const supabase = getSupabase();
    if (!supabase || !yeniAlanAd.trim()) return;
  ...
  async function ustaEkle() {
    const supabase = getSupabase();
    if (!supabase || !yeniUstaAd.trim()) return;
    ...
    <main className="min-h-screen bg-[#02040a] text-slate-300 p-6 font-sans uppercase text-[11px]">
      <div className="max-w-[1800px] mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4 bg-[#0b101d] p-4 rounded-[1.5rem] border border-slate-800/50 overflow-x-auto no-scrollbar">
          ...
          <button onClick={() => setShowAlanModal(true)} className="p-3 bg-blue-600/10 text-blue-500 border border-blue-500/20 rounded-2xl ml-auto mr-4"><Plus size={24}/></button>
        </div>
              ...
              <tbody>
                {ustalar.filter(u => u.alan === aktifAlan).map(usta => (
                  <tr key={usta.id} className="border-t border-slate-800/50 group">
                    <td className="p-6 font-black sticky left-0 bg-[#0b101d] text-slate-200 z-10 border-r border-slate-800">
                        {usta.ad}
                    </td>
                    {gunler.map(g => {
      ...
      {seciliDetay && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[110] backdrop-blur-md">
          <div className="bg-[#0b101d] p-10 rounded-[3rem] w-full max-w-md border border-slate-700 shadow-2xl text-center">
            ...
            <button onClick={() => puantajKaydet('sil')} className="w-full bg-red-600/10 text-red-500 p-4 rounded-2xl font-black mb-4 hover:bg-red-600 hover:text-white uppercase italic text-center">KAYDI SİL</button>
